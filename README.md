...
3. **Summarize** — a digest agent (`gpt-4o-mini`) turns each item into a title and a 2–3 sentence summary.
4. **Rank** — a curator agent (`gpt-4.1`) scores every digest 0–10 against the user profile: interests, expertise level, and preferences such as favoring technical depth over marketing copy.
...
| `app/agent/` | LLM agents: digest, curator, email |
...
4. Set the secrets marked `sync: false` in the Render dashboard: `DATABASE_URL` (the connection string from step 1), `OPENAI_API_KEY`, `MY_EMAIL`, `APP_PASSWORD`, `DIGEST_RECIPIENTS`, `USER_NAME`, `YOUTUBE_CHANNELS`.
...
Roughly one `gpt-4o-mini` call per article and one `gpt-4.1` ranking call per run. On a normal news day that lands in the low cents per digest.
