...
def default_recipients() -> list:
    configured = os.getenv("DIGEST_RECIPIENTS", "")
    recipients = [r.strip() for r in configured.split(",") if r.strip()]
    return recipients or ([MY_EMAIL] if MY_EMAIL else [])
...
def send_email(subject: str, body_text: str, body_html: str = None, recipients: list = None):
    if recipients is None:
        ...
        if not recipients:
            raise ValueError("Set DIGEST_RECIPIENTS or MY_EMAIL to choose where the digest goes")
